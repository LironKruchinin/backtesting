...
//!
//! `synthetic` and `catalog` are implemented. The other modules are
//! **specs encoded as module docs** — read them before implementing M1; they
//! carry decisions made at design time that are easy to get subtly wrong
//! (rolling-window archiving deadlines, roll conventions, timestamp rules).
//!
//! Crate-wide rules:
//! - This is the ONLY crate that touches the network or the filesystem for
//!   market data, and the only place `async`/tokio may appear (bin targets).
