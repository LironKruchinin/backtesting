//! Where a contract's expiry comes from — and, since D-0090, **when it was
//! known**.
//!
...
//! `databento` feature.
//!
//! ## The availability rule (D-0090, superseding D-0046)
//!
...
//! restatement (D-0090). [`contract_cycles_from_definitions`] does not even ask:
...
//! set up, and D-0090 only changed what the roll reader does with the records.
//!
...
impl ExpiryHistory {
    ...
    /// D-0046 behaviour D-0090 replaced: it lets a correction published in 2012
...
impl ExpiryHistoryBuilder {
    ...
    /// behind `ZNZ2011` from the day the archive landed (D-0090).
...
pub(crate) mod imp {
        ...
        F: FnMut(ContractSymbol, Option<Ts>, Ts),
    {
            ...
            .map_err(|e| undecodable(e.to_string()))?
        {
            ...
            // (D-0090). `UNDEF_TIMESTAMP` is `u64::MAX`, so it fails this
    ...
    pub(crate) mod tests {
        ...
        ///
        /// Same availability for every row is the *hard* case for D-0090 and so
        ...
        ///
        /// The fourth column is the whole of D-0090: it is when the vendor said
        /// the third one.
        ...
        // what makes them a restatement rather than a contradiction (D-0090).
        ...
        // The residual refusal, and the only one D-0090 keeps: two expiries at
