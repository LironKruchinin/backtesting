...
result is stated in terms of a clock the grammar cannot read.

### 6.1 The five unlocks

Four pieces of code move most of the B column into A. The fifth does something
different and more important — it is what makes any grade-A result *mean*
anything. None of the five costs a purchase.

| # | Unlock | What it frees |
|---|---|---|
| 1 | **Time-of-day predicate** (+ session anchors, D-0071 pattern) | H-001, H-002, H-004, H-012, H-014 |
| 2 | **1m→5m / daily resampler** | H-003, H-006, H-009, H-014 |
| 3 | **Volume operand** in the rule grammar | H-004, H-012, H-013 |
| 4 | **Rolling normalizer** (M2.5 point-in-time standardization) | H-004, H-009, H-015 |
| 5 | **Registry pooling across contracts** | *every grade A* — turns a ~60-session triage sample into a verdict sample (§2.2) |

Unlock 5 is not a convenience. Grade A currently means "runnable on one
contract's life", and no sample-adequacy criterion worth registering is
satisfiable at ~60 sessions — so today's A-grade runs are guaranteed to be
killed for sample size, correctly, by the machine. That is the pre-registration
working, but it means **the A column produces no verdicts until pooling
lands**. Registry pooling is the M3 workstream's, and it likely arrives before
any of unlocks 1–4.
