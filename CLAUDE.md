...
- **A contract's expiry is keyed on `max(ts_recv)`, and NEVER on
  `max(expiration)`** (D-0085). The vendor restates 4 contracts of this
...
- **The expiry is filtered against the roll's own decision instant even though
  ...
  (D-0085). Every correction here landed 14–18 months before its roll, so the
...
- **Two expiries for one contract are a REVISION, not a conflict**, and only
  *overlapping* availability windows refuse (D-0085, superseding D-0046's
...
- **The `.c` rule resolves a fixed point without iterating**, and there is no
  "did not converge" error to write (D-0085). `due` depends on the expiry and
...
- **A `.v` roll table records `expiry_source = "none"` even on a root whose
  `definition` file is archived** (D-0085). The volume rule reads no expiries —
...
# needs neither (D-0085); all seven roots build both rules today.
