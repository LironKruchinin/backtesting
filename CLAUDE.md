...
- **`AdjustedPrice` cannot be converted to `Price`, and that is the feature**
  ...
  may not depend on `crucible-data` (D-0073); `crucible_data::continuous::
...
- **Every `Bar` carries a `signal_offset`, and every indicator reads
  `bar.signal_*()` rather than `bar.close`** (D-0073). The four `Price` fields
...
- **A back-adjusted level is not §2.1 lookahead for a shift-invariant strategy,
  and IS for a level-sensitive one** (D-0073). Every bar visible at time `T`
...
- **A position carried across a roll books the raw gap as PnL and pays nothing
  for the roll** (D-0073). A roll is a *position* event and this build models it
...
- **`backtest` narrows a date request to the roll table's span and says so;
  `ContinuousFeed::open` still refuses** (D-0073). `--start 2010-06-06` is a
...
- **`backtest` prints an `INSOLVENT` block instead of hiding the ratios it
  invalidates** (D-0073). There is no margin model, so a replay can carry a
...
# The same command over a CONTINUOUS series (D-0073). `--instrument` takes the
